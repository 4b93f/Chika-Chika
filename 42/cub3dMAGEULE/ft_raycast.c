...
/*   Updated: 2020/08/25 17:39:39 by becentrale       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ft_raycast(t_params *params,t_ray *ray, t_color *color)
{
    ...
    while(x++ < params->screenwidth)
    {
        ...
        //printf("%d\n", ray->side);
        if (ray->side == 0)
            ray->perpwalldist = fabs(ray->mapX - ray->posX + (1 - ray->stepX) / 2) / ray->raydirX;
        else
            ray->perpwalldist = fabs(ray->mapY - ray->posY + (1 - ray->stepY) / 2) / ray->raydirY;
